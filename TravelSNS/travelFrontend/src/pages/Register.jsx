...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setError('');
        const res = await register(username, null, password); // Email is null or removed from args
        if (res.success) {
            // 登録成功したらそのままログイン試行、またはログイン画面へ
            ...
            <div className="relative z-10 max-w-md w-full space-y-8 bg-white/95 p-10 rounded-xl shadow-2xl backdrop-blur-sm border border-theme-secondary/30">
                ...
                <div className="text-center mt-6">
                    <p className="text-sm text-theme-text-light">
                        すでにアカウントをお持ちの方は {' '}
                        <Link to="/login" className="font-semibold text-theme-secondary hover:text-theme-primary transition-colors border-b border-transparent hover:border-theme-primary">
                            ログイン
                        </Link>
                    </p>
                </div>
